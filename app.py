...
class Const(Expr):
    ...
    def deriv(self, var):
        return Const(0), [f"[EML 节点提取 - 常数] $\\frac{{d}}{{d{var}}}({self.val}) \Rightarrow 0$"]
...
class Var(Expr):
    ...
    def deriv(self, var):
        if self.name == var:
            return Const(1), [f"[EML 节点提取 - 变量] $\\frac{{d}}{{d{var}}}({self.name}) \Rightarrow 1$"]
        return Const(0), [f"[EML 节点提取 - 偏导] $\\frac{{d}}{{d{var}}}({self.name}) \Rightarrow 0$"]
...
class Add(Expr):
    ...
    def deriv(self, var):
        ...
        return ld + rd, ls + rs + [f"[EML 拓扑合并 - 加法] 线性展开: $\\frac{{d}}{{d{var}}}({self.left} + {self.right}) \Rightarrow {ld} + {rd}$"]
...
class Mul(Expr):
    ...
    def deriv(self, var):
        ...
        return res, ls + rs + [f"[EML 拓扑合并 - 乘法] 莱布尼茨展开: $\\frac{{d}}{{d{var}}}({self.left} \\cdot {self.right}) \Rightarrow ({ld})({self.right}) + ({self.left})({rd})$"]
    def eval(self, env): return self.left.eval(env) * self.right.eval(env)
...
class Pow(Expr):
    ...
    def deriv(self, var):
        if isinstance(self.right, Const):
            ...
            return res, ls + [f"[EML 拓扑展开 - 幂] 链式替换: $\\frac{{d}}{{d{var}}}({self.left}^{{{n}}}) \Rightarrow {n}{self.left}^{{{n-1}}} \\cdot ({ld})$"]
...
class Exp(Expr):
    ...
    def deriv(self, var):
        ...
        return res, id_steps + [f"[EML 拓扑展开 - 指数] 链式替换: $\\frac{{d}}{{d{var}}}(e^{{{self.inner}}}) \Rightarrow e^{{{self.inner}}} \\cdot ({id_res})$"]
...
class Ln(Expr):
    ...
    def deriv(self, var):
        ...
        return res, id_steps + [f"[EML 拓扑展开 - 对数] 链式替换: $\\frac{{d}}{{d{var}}}(\\ln({self.inner})) \Rightarrow \\frac{{1}}{{{self.inner}}} \\cdot ({id_res})$"]
...
class Div(Expr):
    ...
    def deriv(self, var):
        ...
        return res, ls + rs + [f"[EML 拓扑合并 - 除法] 商展开: $\\frac{{d}}{{d{var}}}(\\frac{{{self.left}}}{{{self.right}}}) \Rightarrow \\frac{{({ld}){self.right} - {self.left}({rd})}}{{{self.right}^2}}$"]
    def eval(self, env): return self.left.eval(env) / self.right.eval(env)
